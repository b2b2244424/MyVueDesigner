...
export default class CommonContainerMixins extends Vue {
  ...
  @Watch("components", { deep: true, immediate: true })
  componentsChanged() {
  ...
  public onDragOver(event: any) {
    console.log("CommonContainerMixins.onDragOver");
  }

  public onDrop(event: any) {
    console.log("CommonContainerMixins.onDrop");
  }

  public refreshChildren() {
    // console.log("refreshChildren");
    // if (this.components.length > 0) {
    //   console.log("refreshChildren2");
    //   let child1: DragData = this.components[0];
    //   let id: string = child1.useless;
    //   child1.id = (this as any).$uuid();
    //   setTimeout(() => {
    //     child1.useless = id;
    //   }, 20);
    // } else {
  ...
  created() {
    (this as any).$eventBus.$on(
      BusEvents.REFRESH_CHILD_COMPONENT,
  ...
  public clickEvent(event: any) {
    ...
    // console.log("CommonContainerMixins.clickEvent");
    if (this.isClick) {
  ...
  public mouseDownStartTime: number = 0;
